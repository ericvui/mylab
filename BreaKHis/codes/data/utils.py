...
def predict(model,test,interval_print=50):
    ...
    i = 0
    result = []
    for tst_image, tst_labels, tst_size in test:
        i = i + 1
        y_pred_value = model.predict_on_batch(tst_image)
        pred_label = int(y_pred_value > 0)
        true_label = np.int(tst_labels)
        ...
        if i % interval_print == 0:
            print("[Total testing images: {} ] [2 class accuracy {}%] [40:{}/{}][100:{}/{}][200:{}/{}][400:{}/{}]"
                  . format(i,(total_accuracy/i) * 100,size40_pred,size40,size100_pred,size100,size200_pred,size200,size400_pred,size400))
        #result.append({'size':tst_size,'true_lbl':true_label,'pred_score':y_pred_value,'pred_lbl':pred_label,'acc':accuracy})
...
def train_and_test_model(model, input_ds_filename,ratio=0.7, img_file_type="*.png",dataset_volume=7909, epochs=5,batch_size=16, print_interval=20 , output_filename="C:\\model.h5"):
    ...
    for epoch in range(epochs):
        batch_i = 0     
        t_losses = []
        datasets = load_files(input_ds_filename,ratio,"*.png")
        dataset_train, dataset_test = datasets[0],datasets[1]
        dataset_train = dataset_train.batch(batch_size)
        for x_train,y_train,_ in dataset_train:
            t_loss = model.train_on_batch(x_train, y_train)
            t_losses.append(t_loss)
        ...
        #run test per epoch 
        avg_test_tmp_acc,s1,s2,s3,s4 = predict(model,dataset_test,interval_print=50)
